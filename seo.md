...
**Pages reviewed (5):** `/` (Homepage), `/booking`, `/statestik`, `/handelsbetingelser`, plus checks of `/robots.txt` and `/sitemap.xml`
...
| `https://heybox.dk/handelsbetingelser` | Terms & conditions | Has own `metadata.title`; contains the only real company/contact info (`HeyBox ApS`, `hey@heybox.dk`) |
...
| URL structure | Clean and readable (`/booking`, `/handelsbetingelser`). But `/statestik` is **misspelled** (should be `/statistik`) - hurts keyword relevance and looks unprofessional. | 🟡 Needs Attention |
...
| Internal links | Weak. Almost every link points to `/booking`. The homepage does **not** link to `/statestik` or `/handelsbetingelser` (footer link lists are commented out in `page.tsx`). | 🔴 Needs Attention |
...
| Signal | Finding | Status |
|---|---|---|
| Author / org info | No About page, no team, no "who we are" content. The only company identity lives on `/handelsbetingelser`: *"HeyBox er en service leveret af HeyBox ApS, København, Danmark."* | 🔴 Needs Attention |
| About page | Does not exist. | 🔴 Missing |
| Contact info | Email `hey@heybox.dk` appears on `/handelsbetingelser`. A phone (`+45 70 12 34 56`) and hours exist **only in commented-out footer code** on the statistics page - not live. | 🟡 Needs Attention |
